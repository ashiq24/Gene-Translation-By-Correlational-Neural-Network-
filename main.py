...
if __name__ == "__main__":
    filepath = 'E:\\gtex6\\gtex-skin-wholeblood\\original\\Data\\'# this is directory path #sys.argv[1]
    pca_com = 30 # How many pca components you want to take #int(sys.argv[2])
    copula_num = 8000# Number of samples generated by Copula #int(sys.argv[3])
    ...
    lamda = 20  # A constant for CorNet to balance Correlation loss #float(sys.argv[8])
    ...
    '''
    models = m.Models(model_dim, model_dim, int(model_dim/1.5), int(model_dim/1.8), int(model_dim/2.5), activation,activation_last_layer, lamda)
    model = models.getModel()
    if(copula_num>0):
        d = np.concatenate([lc[:tnum],rc[:tnum]],axis=1)
        cop = Copula(d)
    ...
    '''
    if(train_op==1):
        training.train(left,right,val_left,val_right,800,60,model)
    else:
